...
assert mod.VERSION == "5.3.2-autodebate-broker"
...
assert "auto_debate" in mod.SCHEMA["parameters"]["properties"]
assert "verify_chain" in mod.SCHEMA["parameters"]["properties"]
assert "debate_rounds" in mod.SCHEMA["parameters"]["properties"]
...
skills_probe = mod._execute_safe_tool("skills_list", {"limit": 1})
assert not (isinstance(skills_probe, dict) and skills_probe.get("error") == "tool_invoker_unavailable")
...
print(f"hermes-omnicouncil {mod.VERSION} smoke ok: tools={len(registered)} calls={len(calls)} member_models={data.get('member_model_count')} messages_rounds={data.get('message_rounds')}")
